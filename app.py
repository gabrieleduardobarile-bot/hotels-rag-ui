...
def fetch_csv(url: str, decimal: Optional[str]=None) -> pd.DataFrame:
    r = requests.get(url, timeout=30)
    r.raise_for_status()
    return pd.read_csv(StringIO(r.text), encoding="utf-8", decimal=decimal)

app = FastAPI(title="Hotels RAG Demo")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],        # demo abierto
...
ocup_df: Optional[pd.DataFrame] = None

def load_data():
    global portfolio_df, compras_df, ocup_df
    ...
    if "Habitaciones" in portfolio_df.columns:
        portfolio_df["Habitaciones"] = pd.to_numeric(portfolio_df["Habitaciones"], errors="coerce").fillna(0).astype(int)
...
def source_list():
    return [
        ...
        {"title":"Compras", "url":URL_COMPRAS, "note":"compras, unidades, referencia_producto, precio_unitario, precio_total, hotel, fecha, proveedor"},
        {"title":"Ocupación 2024", "url":URL_OCUP2024, "note":"Marca, Propiedad, Ciudad, País, Tipo, YTD, Ene...Dic"},
    ]
